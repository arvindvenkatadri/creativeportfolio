...
### 1.Graph using geometric objects. I used the diamonds dataset for this graph. In the graph, I plotted the carat of the diamond on the x axis and the price on the y axis. The cut is depicted in different colours and the color of each diamond is depicted in different shapes.
...
```{r}
library(prettymapr)
bbox_thane <- prettymapr::searchbbox(querystring = "Thane, India")
...
tm_shape(dat_thane_H) + tm_lines(col = "dodgerblue", lwd = 1) + tm_shape(dat_thane_R) + tm_dots(size = 0.5, col = "pink", shape = 20) + tm_text("name", auto.placement = TRUE, size = 0.40 )+
  ...
  tm_scale_bar(width = 0.25, position = c("left", "bottom"), text.size = 0.35) +
...
### 3.Network Graphs for Bojack Horseman. I created two datasets - one with nodes and the other with edges.The nodes dataset consists of the names, sex, species and job. The edges dataset consists of the number of times a conversation has taken place between each character has a conversation.
